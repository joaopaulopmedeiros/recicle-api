...
class Desafios extends CI_Controller
{
    ...
    /*function inserir() 
    {
        ...
        if($this->form_validation->run()) 
        {
            ...
            {
                if ($this->desafio_model->verificarDocCadastrado($doc) == null)
                {
                    if ($this->desafio_model->verificarEmail($login) == null)
                    {
                        $data = array(
    ...
    function fetch_single()
    {
        ...
        {
            $data = $this->desafio_model->fetch_single_user($this->input->post('id'));
            foreach($data as $row) {
    ...
    function update()
    {
        ...
        if($this->form_validation->run())
        {
            ...
            $this->desafio_model->update_api($this->input->post('docCadastrado'), $data);
            $array = array(
                'success'  => true
            );
        }
    ...
    function delete()
    {
        if($this->input->post('docCadastrado'))
        {
            if($this->desafio_model->delete_single_user($this->input->post('docCadastrado'))) {
                $array = array(
                    'success' => true
                );
            }
